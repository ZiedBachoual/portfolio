<template>
  <nav class="navbar navbar-expand-lg navbar-dark fixed-top" :class="{ 'navbar-shrink': !shouldExpand }" id="navbar">
    <div class="container">
      ...
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbar-collapse-container"
        aria-controls="navbar-collapse-container" aria-expanded="false" aria-label="Toggle navigation">
      ...
      <div class="collapse navbar-collapse" id="navbar-collapse-container">
        <ul class="navbar-nav text-uppercase ms-auto py-2 py-lg-0">
          <template v-for="menuOption in navbarData.menuOptions">
            <!-- Nav Item -->
            <li class="nav-item">
              <a class="nav-link" :class="{ 'active': displayingSectionId === menuOption.id }"
